...
1. **Get Audio:** Download your own audio clip from online (This could be in an MP3 or WAV format)
...
2. Ensure you name them with the prefix "audio(NUMBER).wav", beginning at audio0.wav, then audio1.wav, then audio2.wav and so on
...
**Important Note:** Flashing the filesystem will completely overwrite any files currently stored on the chip. Ensure all default files your robot's source code expects to find remain in the `data/` folder alongside your custom sounds before hitting upload.